...
public class JUnitManagedIntegrationTest {
    ...
    private static final String MOCKS_WIREMOCK_PATH = "/mocks/wiremock";
    ...
    public void givenJUnitManagedServer_whenMatchingURL_thenCorrect() throws IOException {
        ...
        stubFor(get(urlPathMatching("/mocks/.*"))
        ...
        HttpGet request = new HttpGet(String.format("http://localhost:%s/mocks/wiremock", port));
        HttpResponse httpResponse = httpClient.execute(request);
        String stringResponse = convertHttpResponseToString(httpResponse);

        verify(getRequestedFor(urlEqualTo(MOCKS_WIREMOCK_PATH)));
    ...
    public void givenJUnitManagedServer_whenMatchingHeaders_thenCorrect() throws IOException {
        stubFor(get(urlPathEqualTo(MOCKS_WIREMOCK_PATH))
        ...
        verify(getRequestedFor(urlEqualTo(MOCKS_WIREMOCK_PATH)));
        assertEquals(777, httpResponse.getStatusLine().getStatusCode());
    ...
    public void givenJUnitManagedServer_whenMatchingBody_thenCorrect() throws IOException {
        stubFor(post(urlEqualTo(MOCKS_WIREMOCK_PATH))
                .withHeader("Content-Type", equalTo(APPLICATION_JSON))
                .withRequestBody(containing("\"testing-library\": \"WireMock\""))
                .withRequestBody(containing("\"creator\": \"Sandis\""))
        ...
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpPost request = new HttpPost(String.format("http://localhost:%s/mocks/wiremock", port));
        request.addHeader("Content-Type", APPLICATION_JSON);
        request.setEntity(entity);
        HttpResponse response = httpClient.execute(request);

        verify(postRequestedFor(urlEqualTo(MOCKS_WIREMOCK_PATH))
                .withHeader("Content-Type", equalTo(APPLICATION_JSON)));
    ...
    public void givenJUnitManagedServer_whenNotUsingPriority_thenCorrect() throws IOException {
        stubFor(get(urlPathMatching("/mocks/.*")).willReturn(aResponse().withStatus(200)));
        stubFor(get(urlPathEqualTo(MOCKS_WIREMOCK_PATH)).withHeader("Accept", matching("text/.*")).willReturn(aResponse().withStatus(503)));
    ...
    public void givenJUnitManagedServer_whenUsingPriority_thenCorrect() throws IOException {
        stubFor(get(urlPathMatching("/mocks/.*")).atPriority(1).willReturn(aResponse().withStatus(200)));
        stubFor(get(urlPathEqualTo(MOCKS_WIREMOCK_PATH)).atPriority(2).withHeader("Accept", matching("text/.*")).willReturn(aResponse().withStatus(503)));
